 ...
 */
function combineDataFrames(transformed: DataFrame, remaining: DataFrame | null): DataFrame {
  if (remaining === null) {
    return transformed
  }
  ...
  if (transformedRecords.length !== remainingRecords.length) {
    throw new Error(
      `[combineResults]: Row count mistmatch. Transformed: ${transformedRecords.length}, Remaining: ${remainingRecords.length}`,
    )
  }
