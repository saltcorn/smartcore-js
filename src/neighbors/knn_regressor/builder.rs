...
use super::{
  factory::{self, KNNRegressorFactory},
...
/// KNNRegressorBuilder allows you to set the parameters to be used to initialize a new KNNRegressor instance when you call
/// .build() on it.
#[napi(js_name = "KNNRegressorBuilder")]
...
impl KNNRegressorBuilder {
  ...
  #[napi]
  pub fn build(&mut self) -> Result<KNNRegressor> {
    ...
    let params = factory::NewParameters {
      ...
      knn_regressor_parameters: SetParametersParams {
        k: self.k,
    ...
    Ok(KNNRegressor {
      inner: KNNRegressorFactory::create(params)?,
      fit_data_x_type,
