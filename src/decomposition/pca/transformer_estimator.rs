...
use paste::paste;
use smartcore::{
  decomposition::pca::PCA as LibPCA, linalg::basic::matrix::DenseMatrix as LibDenseMatrix,
};
...
macro_rules! transformer_estimator_impl {
  (x_type: $x:ty) => {
    paste! {
        pub type [<PCA $x:upper>] = LibPCA<$x, LibDenseMatrix<$x>>;

        impl Transformer for [<PCA $x:upper>] {
