macro_rules! define_and_base_impl {
  (
    feature_type: $feat:ty,
    distance_type: $dist:ty,
    distance_export_type: $dist_napi:ty
  ) => {
    paste! {
        #[napi(js_name=""[<DBSCAN $feat:upper $dist_napi  Parameters>]"")]
        #[derive(Debug, Clone)]
        pub struct [<DBSCAN $feat:upper $dist_napi  Parameters>] {
            inner: LibDBSCANParameters<$feat, $dist>,
        }

        #[napi]
        impl [<DBSCAN $feat:upper $dist_napi  Parameters>] {
            #[napi]
            pub fn with_min_samples(&mut self, min_samples: i64) {
                self.inner = self.inner.to_owned().with_min_samples(min_samples as usize)
            }

            #[napi]
            pub fn with_algorithm(&mut self, algorithm: KNNAlgorithmName) {
                self.inner = self.inner.to_owned().with_algorithm(algorithm.into());
            }

            #[napi]
            pub fn with_eps(&mut self, eps: f64) {
                self.inner = self.inner.to_owned().with_eps(eps);
            }

            pub fn owned_inner(&self) -> LibDBSCANParameters<$feat, $dist> {
                self.inner.to_owned()
            }

            #[napi]
            pub fn serialize(&self) -> Result<Buffer> {
                let encoded = encode_to_vec(&self.inner, standard())
                    .map_err(|e| Error::new(Status::GenericFailure, format!("{}", e)))?;
                Ok(Buffer::from(encoded))
            }
        }
    }
  };
}
